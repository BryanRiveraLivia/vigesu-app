...
export default function Home() {
  ...
  const ensureQuickBooksConnected = async () => {
    try {
      ...
      // Validar URL antes de abrir (previene open redirect)
      const ALLOWED_QB_DOMAINS = ["appcenter.intuit.com", "accounts.intuit.com"];
      try {
        const parsedUrl = new URL(url);
        if (!ALLOWED_QB_DOMAINS.includes(parsedUrl.hostname)) {
          toast.error("URL de QuickBooks no válida.");
          setQbStatus(STATUS_QB_ERROR);
          setQbErrorMessage("URL de QuickBooks no válida.");
          return;
        }
      } catch {
        toast.error("URL de QuickBooks malformada.");
        setQbStatus(STATUS_QB_ERROR);
        return;
      }
    ...
    const intervalId = window.setInterval(async () => {
      ...
      try {
        ...
        if (accessToken && accessToken.trim() !== "") {
          ...
          if (mounted) router.push("/dashboard/documents/work-orders");
          return;
        }
  ...
  const onSubmit = async (data: LoginData) => {
    ...
    try {
      // Bypass solo disponible en desarrollo con env var habilitada
      if (
        process.env.NEXT_PUBLIC_ENABLE_MOCK_LOGIN === "true" &&
        data.userName === "admin" &&
        data.password === "123456"
      ) {
        setCookie("auth-token", "mock-token-admin", { path: "/" });
        setAuth({
        ...
        toast.success(`${tToasts("ok")}: Bypass admin activado`);
        router.push("/dashboard/documents/work-orders");
        return;
      }
